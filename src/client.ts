...
export class EisClient {
	 ...
	 *
	 * When optional quenching parameters are provided, the simulation output will include
	 * comprehensive quenching calculations (water flow, air flow, cooling rates, system capabilities).
	 *
	 ...
	 *
	 * Optional quenching parameters (include these to get quenching calculations in output):
	 *     - `alloyCoolingRatio?: number` - Alloy cooling ratio for quenching
	 *     - `waterCapacityQuenching?: number` - Water capacity for quenching system
	 *     - `airQuenchingCapacity?: number` - Air quenching capacity
	 *     - `airLeadOutRunOutTableCapacity?: number` - Air capacity for lead out run out table
	 *     - `quenchingLength?: number` - Length of quenching zone
	 *
	 ...
	 *
	 * Optional quenching output fields (included when quenching parameters are provided in input):
	 *     - `quenchingSystemCapable?: number` - Whether the quenching system is capable (0 or 1)
	 *     - `waterFlowSetting?: number` - Calculated water flow setting
	 *     - `airFlowSetting?: number` - Calculated air flow setting
	 *     - `temperatureProfileAfterQuenching?: number` - Temperature after quenching (°C)
	 *     - `coolingTime?: number` - Cooling time in seconds
	 *     - `afterQuenchingCoolingRate?: number` - Cooling rate after quenching (°C/s)
	 *
	...
	/**
	 * Get aging table data containing alloy specifications and heat treatment parameters
	 *
	 * @returns Promise<ApiResponse<AgingTableOutput>> - API response with either:
	 *   - Success: `{ data: AgingTableOutput, success: true, error: null }` where AgingTableOutput
	 *     is an array of AgingTableEntry objects containing alloy data and aging parameters
	 *   - Error: `{ data: null, success: false, error: string }` - Contains error message
	 */
	public async getAgingTable(): Promise<ApiResponse<AgingTableOutput>> {
		try {
